...
-- 2.3 Create Salon Operating Hours (fixed approach)
WITH salon_hours_data AS (
  SELECT 
    s.id as salon_id,
    day_num as day_of_week,
    CASE 
      WHEN day_num = 0 THEN '10:00'::time -- Sunday
      WHEN day_num = 6 THEN '09:00'::time -- Saturday
      ELSE '09:00'::time -- Monday-Friday
    END as open_time,
    CASE 
      WHEN day_num = 0 THEN '18:00'::time -- Sunday
      WHEN day_num = 6 THEN '20:00'::time -- Saturday
      ELSE '19:00'::time -- Monday-Friday
    END as close_time,
    CASE 
      WHEN s.name = 'Classic Barbershop' AND day_num = 0 THEN true -- Closed Sundays
      ELSE false
    END as is_closed
  FROM public.salons s
...
SELECT salon_id, day_of_week, open_time, close_time, is_closed FROM salon_hours_data;
